...
if __name__ == '__main__':
    ...
    opt_dis = torch.optim.Adam(dis.parameters(),lr=lr_d,betas=(0, 0.9))
    ...
    for eps in tqdm(range(eps_rec,episode)): 
        epsilon = ep_episode(eps)
        step_num = 0
        ...
        while step_num <= epoch_step_num:
            ...
            for i in range(update_num):
                ...
                next_act_dist,_,_ = gen(ego_train_next_state)
                ...
                q_target = (reward_weight*ego_rew  + gamma * (1-ego_train_done.reshape(-1,1)) * (q_val.reshape(-1,1)- alpha * next_act_log_pro.reshape(-1,1))).reshape(-1,1) #
                ...
                loss_gen = -torch.mean(q_new_val - new_ego_pro*alpha)+ mean_weight * (mean**2).mean() + std_weight * (std**2).mean()# + binary_cross_entropy_with_logits(bat_ego_score.detach(), torch.ones_like(bat_ego_score)) * bat_ego_pro)
                loss_gen.backward()
                opt_gen.step()
                ...
                alpha_loss.backward()
                opt_alpha.step()
                #log_alpha = torch.clamp(log_alpha, -100, 0)
        ...
        print(f"mean is {np.mean(mean_)}")
        print(f"std is {np.mean(std_)}")
